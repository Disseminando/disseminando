...
if (isset($_POST[$MM_flag])) {
  $MM_dupKeyRedirect="fin_cad_categoria.php";
  $loginUsername = $_POST['nome_cat'];
  $LoginRS__query = sprintf("SELECT cat_nome FROM fin_categoria WHERE cat_nome=%s", GetSQLValueString($loginUsername, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO fin_categoria (cat_nome, cat_usuario) VALUES (%s, %s)",
                       GetSQLValueString($_POST['cat_nome'], "text"),
                       GetSQLValueString($_POST['cat_usuario'], "text"));

  mysql_select_db($database_con01, $con01);
  $Result1 = mysql_query($insertSQL, $con01) or die(mysql_error());

  $insertGoTo = "fin_cad_categoria.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$maxRows_ls_categoria = 30;
$pageNum_ls_categoria = 0;
if (isset($_GET['pageNum_ls_categoria'])) {
  $pageNum_ls_categoria = $_GET['pageNum_ls_categoria'];
}
$startRow_ls_categoria = $pageNum_ls_categoria * $maxRows_ls_categoria;

$usuario=$_SESSION['MM_Username'];

mysql_select_db($database_con01, $con01);
$query_ls_categoria = "SELECT cat_id, cat_nome FROM fin_categoria WHERE cat_usuario='$usuario' ORDER BY cat_id DESC";
$query_limit_ls_categoria = sprintf("%s LIMIT %d, %d", $query_ls_categoria, $startRow_ls_categoria, $maxRows_ls_categoria);
$ls_categoria = mysql_query($query_limit_ls_categoria, $con01) or die(mysql_error());
$row_ls_categoria = mysql_fetch_assoc($ls_categoria);

if (isset($_GET['totalRows_ls_categoria'])) {
  $totalRows_ls_categoria = $_GET['totalRows_ls_categoria'];
} else {
  $all_ls_categoria = mysql_query($query_ls_categoria);
  $totalRows_ls_categoria = mysql_num_rows($all_ls_categoria);
}
$totalPages_ls_categoria = ceil($totalRows_ls_categoria/$maxRows_ls_categoria)-1;
?>
<!DOCTYPE HTML>
<html>
<head>
    ...
    <div class="c9"><br />
      ...
      <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
        <table border="0" cellpadding="0" cellspacing="0">
          <tr valign="baseline">
            <td width="13%" valign="baseline" nowrap>Categoria:</td>
            <td width="87%"><input name="cat_nome" type="text" value="" size="20" maxlength="40"></td>
          </tr>
          <tr valign="baseline">
            <td width="13%" valign="baseline" nowrap>Cadastrador:</td>
            <td width="87%"><input name="cat_usuario" type="text" value="<?php $usuario =$_SESSION['MM_Username'];echo "$usuario";?>" readonly="readonly"></td>
          </tr>
          <tr valign="baseline">
      ...
      <br>
       <h5>Listagem de Categorias</h5>
        ...
        <table border="1" cellpadding="10" cellspacing="10">
          <tr>
            <td bgcolor="#999999"><font color="#000099" size="4">Código</font></td>
            <td bgcolor="#999999"><font color="#000099" size="4">Categoria</font></td>
          </tr>
          <?php do { ?>
            <tr>
              <td><?php echo $row_ls_categoria['cat_id']; ?></td>
              <td><?php echo $row_ls_categoria['cat_nome']; ?></td>
            </tr>
            <?php } while ($row_ls_categoria = mysql_fetch_assoc($ls_categoria));            ?>
        </table>
<br>
        <br>
        <br>
